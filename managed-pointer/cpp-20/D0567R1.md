...
* The `managed_ptr` synchronisation member function `get()` and the **executor** class synchronisation member functions `get()`, `then_get()`, `put()` and `then_put()` have been removed, leaving only the global synchronisation functions `get()`, `then_get()`, `put()` and `then_put()`.
...
* The `managed_ptr` member alias `future_type` has been removed and the global synchronisation functions `get()`, `then_get()`, `put()` and `then_put()` now return `executor_future_t`.
...
* The `managed_ptr` now has the member function `is_host_based()` to query if the `managed_ptr` supports host based capabilities.
...
* The `managed_ptr` member function `is_host_based` is now a static constexpr member variable.
* The global synchronisation functions `get()` and `then_get()` no longer take an **executor** parameter, as the associated **executor** can be inferred from the `managed_ptr` parameters.
* The global synchronisation operations `sync` and `then_sync` which take an **execution channel** have been added.
...
Much valuable feedback was given dsince the last revision of this paper, this section will identify the issues that were raised and points to their respective solutions.
...
### Optimising sycnhornisation between execution contexts

It was suggested that it be possible for an implementation to optimise synchronisation of a `managed_ptr` between particular execution contexts rather than having to synchronise via a host execution context. A solution that was being discussed but didn't make it into the first revision of the paper was the concept of channels. This is now supported in this paper and is described in section 3.5.
...
It was asked what would happen if `malloc()` were to be called on a GPGPU, and the answer is this cannot be supported due to hardware limitations on some devices such as GPUs. This question of *execution context* capabilities is described in section 3.3.5.
...
This paper proposes an extension to the unified interface for execution proposal [1] to facilitate the management and sycnchronisation of a memory allocation across multiple memory region(s). This addition is in the form of the class template `managed_ptr`; similar to the `std::shared_ptr` but with the addition that it can share its memory allocation across the memory region of it's host execution context where it is constructed and the memory region(s) of one or more other host or remote execution contexts.

The aim of this paper is to begin an exploratory work into designing a unified interface for data movement. There are many different architectures and data flow models to consider when designing such an interface so it is expected that this paper will serve only as a beginning.
...
A device in this respect can be any architecture that exists within a system that is C\+\+ programmable; this can include CPUs, GPGPUs, APUs, FPGAs, DSPs, NUMA nodes, I/O devices and other forms of accelerators.
...
This proposal is influenced by many heterogeneous and distributed programming models; including SYCL [2], HPX [3], KoKKos [4], Raja [5], HAM-Offload [?] and MYU [?].
...
These memory regions can be categorised into two groups. Firstly memory architectures which have a single unfied address space, that is a single area of addressable memory that is accessible across multiple devices, this can optionally be supprted in hardware by devices having the same physical memory or in software by having a cache coherancy layer accessing a shared virtual address space. Secondly memory architectures which have multiple address spaces, therefore having separate discrete areas of addresable memory, often in different physical memory.
...
Different memory architectures, both unified and discrete; may support granularity at different levels. This can either be at the course grained level; where synchronisation between memory regions is performed at the execution function level, or this can be at the fine grained level; where synchronisation between memory regions is performed at the instruction level, allowing access to memory from different devices concurrently, using atomics to specify read and write order.
...
There are also many different data flow models that can be used to describe data and data dependenies in relation to functions of execution. Though it would be impossible to cover all possible models, it's important to make a unified inteface as flexible and utilisable as possible, therfore facilitating a wide range of data flow models.

One such data flow model that C\+\+ is moving towards is that of futures and continuations; the idiom that the invocation of an asycnhornous task returns a future object that represents a dependency on the completion of that task that can be waiting or used to create a continuation task. This allows tasks to be linked together by their dependencies on each other.

Another data flow model is one that uses a runtime to create a graph of tasks, which can be optimised.

Another data flow model is one which simply submits work to be executed and waits for it all to complete, typically called fire and forget execution.

### 2.4. Must integrate seemlessly with exexcutors

As data movement is tightly coupled with executtion, it is nessesary for a unified interface for data movement to be integrated with the unified interface for execution. This means such a unified interface must integrate seemlessly with the current design philosophy of executors, in that it must be compatible for the range of control structures which the executors abstraction supports and must support te range of platforms which executors can target.
...
A typical example of this is that a CPU cluster would have no problem allocating a `managed_ptr` on any ne of it's nodes and peforming synchronisation operations on any node, where as with a typical GPU all allocation and sycnhronisation must be done from a host CPU device priory to execution.
...
class executor {
  ...
  /* aliases */
  using global_memory_allocator_type = __global_memory_allocator_type__;

  /* member functions */
  global_memory_allocator_type global_memory_allocator() const;
...
public:
  ...
  using executor_type          = __executor_type__;
  using default_allocator_type = executor_type::global_memory_allocator_type;

  /* constexpr values */
  static constexpr bool is_host_based = __is_host_based__;

  /* constructors */
  managed_ptr(pointer); // (1)
  template <typename Allocator>
  managed_ptr(Allocator allocator = default_allocator_type{}); // (2)
  ...
  bool is_allocated(Executor = executor_type{}) const;
  template <typename Executor>
  bool is_accessible(Executor = executor_type{}) const;
...
class executor_future_t {
  ...
  future_type_t<Executor, void> put(Executor, ManagedPtrN...);
  template <typename... ManagedPtrN>
  future_type_t<Executor, void> get(ManagedPtrTN...);
  template <typename SynchronisationChannel, typename SrcExecutor,
            typename DestExecutor, typename... ManagedPtrN>
  future_type_t<Executor, void>
  sync(SynchronisationChannel = __default_synchronisation_channel__{},
       SrcExecutor, DestExecutor, ManagedPtrN...);

  /* allocation member functions */
  template <typename Executor, typename... ManagedPtrN>
  future_type_t<Executor, void> allocate(Executor, ManagedPtrN...);
  template <typename Executor, typename... ManagedPtrN>
  future_type_t<Executor, void> deallocate(Executor, ManagedPtrN...);
...
future_type_t<Executor, void> then_put(Executor, Predicate, ManagedPtrN...);
template <typename... ManagedPtrN>
future_type_t<Executor, void> get(ManagedPtrTN...);
template <typename Predicate, typename... ManagedPtrN>
future_type_t<Executor, void> then_get(Predicate, ManagedPtrN...);
template <typename SynchronisationChannel, typename SrcExecutor,
          typename DestExecutor, typename... ManagedPtrN>
future_type_t<Executor, void>
sync(SynchronisationChannel = __default_synchronisation_channel__{},
...
future_type_t<Executor, void>
then_sync(SynchronisationChannel = __default_synchronisation_channel__{},
...
future_type_t<Executor, void> allocate(Executor, ManagedPtrN...);
template <typename Executor, typename Predicate, typename... ManagedPtrN>
future_type_t<Executor, void> then_allocate(Executor, Predicate, ManagedPtrN...);
template <typename Executor, typename... ManagedPtrN>
future_type_t<Executor, void> deallocate(Executor, ManagedPtrN...);
template <typename Executor, typename Predicate, typename... ManagedPtrN>
future_type_t<Executor, void> then_deallocate(Executor, Predicate,
                                              ManagedPtrN...);
...
In addition to a number of *execution agents* an *execution context* must also encapsulate at least one memory region that is concurrently accessible by all *execution agents* of any single invcation of an execution function. It is not required that the memory region be concurrently accessible to *execution agents* of another invocation of an execution function or *execution agents* executing on a different *execution context*.

> This requirement currently satisfies the course grained level of synchronisation described in requirement 2. These requirements will be loosened in a later iteration of this paper when a suitable interface for fine grained sycnrhonisation is included. The curent proposal for this is described in section 5.1 as future work.

#### 3.2.2. Executor Global Memory Allocator

The *execution context* must also provide an alias to an allocator type called `global_memory_allocator_type` that is capable of allocating memory in it's memory region, and a member function for retrieving an instance of this type called `global_memory_allocator()`.

### 3.3. Managed Pointer Class Template

The proposed addition to the standard library is the `managed_ptr` class template. The `managed_ptr` is a smart pointer which has ownership of a contiguous managed allocation of memory that can be shared between any number of devices  represented by their respective *execution contexts*.
...
> Another solution for read-only access was considered where access to a `managed_ptr` instance could be dynamically specified via properties using the proposed `array_ref` interface [10]. However it was felt that this approach would be too verbose and would introduce unnessesary overhead for casess which did not require the properties to be specified. Additionally as many implementation would wish to make use of read-only partitions of memory this could require an implementation to copy in an out of this memory partition dynamically which could introduce opaque latency in synchronisation.
...
The default constructor for `T` must not be called by the `managed_ptr` during allocation.
...
As the `managed_ptr` can be passed between and used on different devices it can therefore have different capabilities depending on where it is currently being used. The `managed_ptr` will likely be compiled separately or a least have different restrictions imposed on the code that is executed depending on where it s used. It is therefore important to be able to determine what capabilities the `managed_ptr` supports where it is being used.

The major distinction is whether or not the `executor_type` of a `managed_ptr` is a *host* which or a *remote*. If the `executor_type` is a *host* then it supports host based capabilities; being able to perform allocation and sycnhronisation operations, if it is a *remote* it does not. Whether the `executor_type` of a `managed_ptr` is a *host* can be queried with the static constexpr bool `is_host_based`.

> For the purposes of this paper the capabilities of host and remote execution contexts simply state that allocation and sycnhronisation operations can only be performed by host execution contexts. In reality the capabilities of execution contexts will be far more complex and will require an depth interface for querying said capabilities. However this problem of language capabilities on execution contexts particularly for heterogeneous architectures is a much larger problem for C\+\+ that goes beyond the scope of this paper and indeed the executors proposal as well.
...
* The `get` operation synchronises the data at the memory region at a particular *execution context* to the memory region at the *execution context* associated with the `managed_ptr`.
* The `put` operation synchronises the data at the memory region at the *execution context* associated with the `managed_ptr` to the memory region at a particular *execution context*.
* The `sync` operation synchronises the data at the memory region at one particular *execution context* to the memory region at another particular *execution context*.

> In the previous revision of this paper it was also possible to implicitly trigger sycnhornisation operations by simplly passing a `managed_ptr` to a control structure such as `async`. However this has been removed due to a limitation that may effect implementations. This is described in section 5.2.

There are a further three sycnhronisation operations; `then_get`, `then_put` and `then_sync`, which are equivelants of `get`, `put` and `sync` but take a predecate future parameter that the operation must wait on before executing. These are used to provide a continuations interface for sycnhronisation operations. Equivelant operations to these are also required as member functions on the future type, this is described further in section 3.4.
...
print(ptrB);
```
*Figure 2: Example of synchronisation operations*
...
### 3.5. Optimising Sychronisation Operations using Synchronisation Channels

Synchronisation channels are objects which represent a connection between two execution contexts, for example between two remote devices or between an I/O device and a remote device. The default sycnhronisation channel which supports sycnhronisation between all execution contexts is that which sycnhronises via the host execution context. However implementors can provide their own synchronisation channels which can be used by the user in synchronisation operations to optimise them.
...
managed_ptr<my_data> ptr(inputStreamContext.global_memory_allocator());
...
```
*Figure 3: Synchronisation Channels Example*

> This is not the most efficient example, it could be improved by double buffering the sycnhornisation and computation, though it demonstrates how channels can be used to avoid sycnhronising via a host.
...
This proposal is not yet in the main paper and so is considered future work as the idea has not yet been fully fleshed out and there are other factors that have to be considered such as the execution context - memory region topology and atomic operations.

### 5.2. Implicit Sycnhronisation

In a previous revision of this paper it was possible to trigger implicit sycnhronisation operations by simply passing a `managed_ptr` to a control structure such as `async`. However this has been removed due to limitations that may effect an implementation. As the current executors interface does not support variadic execution function parameters, implicit an implementation would be required to trigger sycnrhonisation operations on all control structures for `managed_ptr` captures. Firstly this may incur unwanted overhead and secondly this could potentially require static reflection for an optimal implementation. For these reasons this feature has been removed and instead kept as a potential future work that could be incorporated if proven to be implementable without creating overhead.
...
### 5.4. Hierarchical Memory Structures
